...
list* const LST_POISON = (list* const)123;

#define HTableInit(size)                        _HTableInit((size), LOCATION)
#define HTableFind(obj, key, len, dest)         _HTableFind((obj), (key), (len), (dest), #obj, LOCATION)
#define HTableFindAligned(obj, key, len, dest)  _HTableFindAligned((obj), (key), (len), (dest), #obj, LOCATION)
#define HTableInsert(obj, key, len, val)        _HTableInsert((obj), (key), (len), (val), #obj, LOCATION)
#define HTableInsertAligned(obj, key, len, val) _HTableInsertAligned((obj), (key), (len), (val), #obj, LOCATION)
...
inline uint GetListSize(const HTable* obj, const uint n_list){
    if(obj->lists[n_list] == LST_POISON) return 0;

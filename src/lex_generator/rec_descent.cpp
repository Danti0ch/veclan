...
Node* CreateAST(const string* RE_str){
    
    NASSERT(RE_str);

    handler.storage = GetStorage(RE_str->data, StringLen(RE_str));
...
Node* make_node_level(Node* cur_root, char new_root_symb, char right_node_symb){
    ...
    Node* right_ch = NodeCtor(new_root, &right_node_symb, sizeof(char), NODE_PLACE::RIGHT);
